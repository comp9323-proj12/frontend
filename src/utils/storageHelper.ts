...
export function put<T>(key: string, value: T, memory = false): T {
  ...
  store.setItem(
    makeKey(key),
    JSON.stringify(value, (jKey, jValue) =>
      /^\${2}/.test(jKey) ? undefined : jValue,
    ),
  );
...
export const insertSessionStorage = (key, newValue) => {
...
export const patchSessionStorage = (key, newValue) => {
  const value = JSON.parse(sessionStorage.getItem(key));
  if (value) {
    Object.assign(value, newValue);
    sessionStorage.setItem(key, JSON.stringify(value));
